...
# ------------------------------------------------
id_predlozhka = -1001662137857
id_offtop = -1001556322936
# ------------------------------------------------
linkpredlozhka = "https://t.me/NO_SCAM_HighRise"
linkofftop = "https://t.me/+Dz8j2vMmXBtkZmM6"
# ------------------------------------------------
username_predlozhka = 'NO_SCAM_HighRise'
username_channel = 'Higfrise'
# ------------------------------------------------
version = '2.1.5'
system_dir = ["admin", "scam", "db", "db_group"]
# ------------------------------------------------
owner = 5631980944
...
async def start(client, message):
    ...
    try:
        ...
        if result >= 1:
            try:
                ment = f'{message.from_user.mention}'
            except:
                ment = ' '
...
🚫 | /chk [@|ID] - если ли он/а в базе скамеров?
🔍 | /me - я есть в базе?
...
''',
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [InlineKeyboardButton(f'✅НОВОСТНОЙ КАНАЛ✅', url=f'https://t.me/Highrise_newsbaza')],
                        [InlineKeyboardButton(f'МЕНЯ ЗАСКАМИЛИ...', url=f'https://t.me/{username_predlozhka}')],
                        [InlineKeyboardButton(f'НАШИ ЧАТЫ', url=f'https://t.me/HIGHRISEHGRR')],
...
async def globan(client, message):
    db(message.chat.id)
    try:
        client, message, result = await subsess(client, message)
        if result >= 1:
            ids = message.text.split()[1]
            becausee = message.text.split(f"/global {ids} ", maxsplit=1)[1]

            client, message, number = await get_id(client, message, ids)
            ids = str(number)

            useres = message.chat.username
            if useres == None:
                useres = message.chat.id
                if useres > 0:
                    pass
                else:
                    useres += 1000000000000
                useres = abs(useres)
                useres = f"c/{str(useres)}"

            try:
                proofs = f"https://t.me/{useres}/{message.reply_to_message.id}"
                proofes = f"""📝| <a href="{proofs}">ПРУФЫ</a>\n"""
            except:
                proofs = "None"
                proofes = ""

            if Path(f"admin/{str(message.from_user.id)}").is_file():
                await write_DB(f"scam/{ids}", str(proofs + " " + becausee))

                await app.send_message(
                    chat_id=message.chat.id,
                    text=f"""
ㅤ ❗️НОВЫЙ ЧЕЛОВЕК В БАЗЕ❗️
🆔 | <code>{str(ids)}</code>

✏️| ПРИЧИНА: {becausee}
👥| <a href="tg://user?id={str(ids)}">ПРОФИЛЬ</a>
{proofes}

<a href="https://t.me/HGRMATE/203">ЧТО ТАКОЕ АЙДИ?</a> 
""")
...
async def unban(client, message):
    ...
    try:
        ...
        if result >= 1:
            ...
            try:
                os.remove(f"scam/{msg}")
                await app.send_message(message.chat.id, "✅ | Вынес пользователя")
            except Exception as f:
                await app.send_message(message.chat.id, f"⚠ | ОШИБКА {f}, при разбане пользователя")
    except FloodWait as e:
        asyncio.sleep(e.value)


# Команда me
@app.on_message(filters.command(['me', 'chk me']))
    ...
    try:
        ...
        if result >= 1:
            try:
                ...
                resultated = f"""
❗️ | {ment}, ты не правильно ввёл/а команду. 
Как нужно: /chk @PAS_0
По айди:   /chk 5721028095
"""
                await app.send_message(
                    chat_id=message.chat.id,
                    text=f"{resultated}",
...
async def subsess(client, message):
    ...
    try:
        ...
        try:
            ...
            await app.send_message(
                chat_id=message.chat.id,
                text=f"❗️ | {mentiones}, подпишитесь на @{username_channel} для правильной работы бота",
                reply_to_message_id=message.id,
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
...
async def check(client, message, number):
    try:
        ...
        try:
            ...
            ment = f"{usermen.mention} (`{str(number)}`)"
        except:
            ment = f"`{str(number)}`"
        ...
        if Path(f"scam/{number}").is_file():
            ...
            await app.send_video(message.chat.id, random.choice(have_in_base_gif), f'⛔️ | {ment} есть в базе...\n❓ | ПРИЧИНА: {because}\n📝 | ПРУФЫ: {proof}', reply_to_message_id=message.id,
            reply_markup=InlineKeyboardMarkup(
                    [
                        [InlineKeyboardButton(f'СЛИТЬ СКАМЕРА', url=linkpredlozhka)],
                    ]
                )
            )
        else:
            await app.send_video(message.chat.id, random.choice(dont_have_in_base_gif), f'✅ | {ment} нет в базе скамеров', reply_to_message_id=message.id,
                reply_markup=InlineKeyboardMarkup(
                    [
                        [InlineKeyboardButton(f'КУДА ПОБЕЖАЛ?)', url=linkpredlozhka)],
                    ]
                )
            )
